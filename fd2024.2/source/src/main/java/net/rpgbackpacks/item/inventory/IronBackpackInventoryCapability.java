...
public class IronBackpackInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == RpgBackpacksModItems.IRON_BACKPACK.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(36) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != RpgBackpacksModItems.IRON_BACKPACK.get();
			}
